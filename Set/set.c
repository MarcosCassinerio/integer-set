...
Set set_intersecar(Set set1, Set set2) {
    ...
    Set salida = set_crear();
    while (posicion1 < set1->size) {
        posicion2 = 0;
        posicion2 = buscar_inicio_interseccion(set2->intervalArray, set1->intervalArray[posicion1], 1, set2->size);
        while (posicion2 < set2->size) {
            interseccion = interval_interseccion(set1->intervalArray[posicion1], set2->intervalArray[posicion2]);
            if (interseccion) {
...
Set set_restar(Set set1, Set set2) {
    return set_intersecar(set1, set_complemento(set2));
}
...
Set set_complemento(Set set) {
    ...
    for (; posicion < set->size; posicion ++) {
        if (posicion == 0) {
            if (interval_extremo_izq(set->intervalArray[posicion]) != INT_MIN)
                set_insertar_ultimo(&salida, interval_crear(INT_MIN, interval_extremo_izq(set->intervalArray[posicion]) - 1));
        } else
            set_insertar_ultimo(&salida, interval_crear(interval_extremo_der(set->intervalArray[posicion - 1]) + 1, interval_extremo_izq(set->intervalArray[posicion]) - 1));
    }
    if (set->size && interval_extremo_der(set->intervalArray[set->size - 1]) != INT_MAX)
        set_insertar_ultimo(&salida, interval_crear(interval_extremo_der(set->intervalArray[set->size - 1]) + 1, INT_MAX));
    return salida;
}
